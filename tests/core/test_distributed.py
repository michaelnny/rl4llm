...
def fake_dist(monkeypatch):
    ...
    monkeypatch.setattr(dist, "is_initialized", lambda: fake_state["initialized"])
    ...
    monkeypatch.setattr(dist, "get_rank", lambda: int(os.environ.get("RANK", 0)))
    monkeypatch.setattr(
        dist, "get_world_size", lambda: int(os.environ.get("WORLD_SIZE", 1))
    )
    monkeypatch.setattr(dist, "get_backend", lambda: "gloo")
    monkeypatch.setattr(dist, "barrier", lambda: fake_state["calls"].append("barrier"))
    ...
    monkeypatch.setattr(
        ...
        lambda tensor, gather_list, dst: [
            gather_list.__setitem__(i, tensor.clone()) for i in range(len(gather_list))
        ],
    )
    monkeypatch.setattr(
        dist,
    ...
    monkeypatch.setattr(
        dist,
        "gather_object",
        lambda obj, object_gather_list, dst: object_gather_list.__setitem__(0, obj)
    ...
    monkeypatch.setattr(
        ...
        lambda output_objects, obj: [
            output_objects.__setitem__(i, obj) for i in range(len(output_objects))
        ],
    )
    monkeypatch.setattr(dist, "broadcast_object_list", lambda obj_list, src: None)
    monkeypatch.setattr(
        dist,
        "scatter_object_list",
        lambda scatter_object_output_list,
        scatter_object_input_list,
        src: scatter_object_output_list.__setitem__(
            0, scatter_object_input_list[0] if scatter_object_input_list else None
        ),
    )
