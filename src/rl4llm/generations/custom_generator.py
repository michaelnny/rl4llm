...
class CustomLLMGenerator:
    ...
    def _get_next_token_logits(
        ...
        """Get next token logits using KV cache."""
        outputs = self.model(
            input_ids=input_ids,
    ...
    def _sample_next_token(
        ...
        if apply_noise:
            ...
            probs = self._add_dirichlet_noise(probs, noise_eps, noise_alpha)
        else:
    ...
    def _sample_next_tokens(
        ...
        for logits, temp in zip(token_logits, temperature):
            next_token = self._sample_next_token(logits, temp.item(), top_p, apply_noise, noise_eps, noise_alpha)
    ...
    def generate(
        ...
        random_start_eps: float = 0.05,
        random_start_alpha: float = 0.03,
        **kwargs,
        ...
        while cur_len < max_new_tokens:
            ...
            next_token_logits, past_key_values = self._get_next_token_logits(
            ...
            if random_start_steps is not None and random_start_steps > 0:
                apply_noise = (cur_len - prompt_len) < random_start_steps
            
            # print(f"Cur Len: {cur_len}, Apply Noise: {apply_noise}")

            # Sample next tokens
            next_tokens = self._sample_next_tokens(
                next_token_logits, temperature, top_p, apply_noise, random_start_eps, random_start_alpha
            )
