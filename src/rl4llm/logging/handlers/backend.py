...
class BackendHandler(BaseHandler):
    ...
    def __init__(
        ...
        if self.is_master:
            ...
            self._writer = self._setup_backend_writer(enable_wandb, enable_tensorboard)
            self._can_log_flag = self._writer is not None
        else:
            self._logger.debug("Not master rank, skipping backend writer setup.")
    ...
    def _setup_backend_writer(
        ...
        if enable_wandb:
            try:
                ...
                run_id = getattr(self.config, "run_id", wandb.util.generate_id())

                wandb.init(
                    project=getattr(self.config, "project_name", "rl4llm_project"),
                    config=(
                        vars(self.config)
                    ...
                    settings=wandb.Settings(
                        ...
                        log_internal=os.path.join(wandb_dir, "wandb-internal.log"),
        ...
        if enable_tensorboard and not backend_writer:
            try:
                ...
                self._logger.info(f"Initialized TensorBoard. Logs in: {tb_log_dir}")
                try:
    ...
    def log_metrics(self, metrics: Dict[str, Union[float, int]], step: int) -> None:
        ...
        try:
            ...
            if not valid_metrics:
                self._logger.debug(f"No valid finite metrics to log at step {step}")
                return
        ...
        try:
            if self._is_wandb_writer():
                ...
                self._writer.add_text(
                    "configuration/hyperparameters", f"```yaml\n{params_str}\n```", 0
                )
                self._logger.info(
            ...
            self._logger.warning(f"Failed to log hyperparameters to backend: {e}")
