...
def parse_args():
    ...
    parser.add_argument('--max-size', type=int, default=100000, help='Maximum number of samples to process (default: 100000)')
...
def extract_positive_samples(dataset, max_size, tokenizer, max_tokens, num_workers=None, batch_size=100):
    ...
    positive_token_chunks = positive_token_chunks[:max_size]
...
def extract_negative_samples(
    ...
    if intensities is None:
        intensities = [round(x, 2) for x in np.arange(0.1, 0.5, 0.05)]
    if num_workers is None:
        num_workers = cpu_count()
...
def main():
    ...
    token_stats = collect_token_stats(positive_token_samples, negative_token_samples)
