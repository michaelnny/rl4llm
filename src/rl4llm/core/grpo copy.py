...
class GRPOTrainer:
    ...
    def run_evaluation(self):
        ...
        with self.metrics.timer('evaluation'):
            for batch in self.test_loader:
                ...
                _ = self._process_generation_outputs(
                    questions, ground_truths, task_types, inputs.input_ids, outputs.sequences, False
                )
    ...
    def _process_generation_outputs(
        ...
        Args:
            ...
            is_training: Whether this is training (True) or evaluation (False)
        ...
        for i in range(len(completion_texts)):
            ...
            if i in sample_indices or not is_training:
                ...
                self._log_sample_to_tensorboard(
                    tb_tag, task_types[i], questions[i], ground_truths[i], completion_texts[i], total_rewards[i].item()
                )
    ...
    def _log_sample_to_tensorboard(
        self, tag: str, task: str, question: str, ground_truth: str, completion_text: str, reward: float
    ):
        ...
        if self.writer:
            ...
            self.writer.add_text(f'{tag}/sample', formatted_text, self.episode_count)
