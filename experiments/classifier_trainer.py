...
class ClassifierTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        super().__init__(
            config, tokenizer, device, torch_dtype, artifacts_path, logger, 0
        )
    ...
    def _evaluate(self) -> None:
        ...
        # Format for logging
        self._log_stats_to_tensorboard(
            {f'evaluation/{k}': v for k, v in metrics.items()}, step=self.step
        )
    ...
    def _train(self) -> None:
        ...
        total_updates_per_epoch = math.ceil(
            len(self.train_loader) / self.config.gradient_accumulate_steps
        )
        total_steps = (
            self.config.num_epochs * total_updates_per_epoch
        ...
        with torch.autograd.set_detect_anomaly(True):
            for epoch in range(self.config.num_epochs):
                for i, batch in enumerate(self.train_loader):
                    ...
                    output = self.model(input_ids, attention_mask=atten_mask)
                    loss, batch_metrics = self._compute_loss(
                    ...
                    if (i + 1) % self.config.gradient_accumulate_steps == 0 or (
                        i + 1
                        ...
                        if (
                            ...
                            save_dir = os.path.join(
                                self._checkpoint_dir, f"step_{self.step}"
                            )
