...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Build coherent dataset for training classifier'
    )
    parser.add_argument(
        '--model-names',
        type=str,
    ...
    parser.add_argument(
        ...
        help='The target model we want to fine-tune',
    )
    parser.add_argument(
        '--batch-size',
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help='Maximum number of samples per tasks to process (default: 1000)',
    )
    parser.add_argument(
        '--max-tokens',
        type=int,
        default=4000,
        help='Maximum number of tokens per sample (default: 4000)',
    )
    parser.add_argument(
        '--split-ratio',
        type=float,
...
def generate_positive_samples(
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        for j, sample in enumerate(batch):
            ...
            mask = (completion_ids != tokenizer.eos_token_id) & (
                completion_ids != tokenizer.pad_token_id
            )
...
def generate_negative_samples(
    ...
    for sample in source_dataset:
        ...
        if random.random() < repetition_probs and len(completion_tokens) > 50:
            ...
            manipulated_completion_tokens = add_repetition(
        ...
        manipulated_completion_text = tokenizer.decode(
            manipulated_completion_tokens
        )
        negative_sample = {
...
def manipulate_tokens(
    ...
    if position == 'beginning':
        ...
        indices_to_manipulate = random.sample(
            range(num_tokens), min(num_to_manipulate, num_tokens)
        )
        ...
        indices_to_manipulate = random.sample(
            indices_to_manipulate, min(num_to_manipulate, num_tokens)
        )
...
def extract_completion_from_item(item: Dict) -> str:
    ...
    if 'completion' in item and isinstance(item['completion'], str):
        ...
        'generations' in item
        and isinstance(item['generations'], list)
        and len(item['generations']) >= 1
    ):  # from OpenR1 Math
        completion = random.choice(item['generations'])
    elif 'messages' in item and isinstance(
        item['messages'], list
    ):  # standard SFT chat
        last_turn = item['messages'][-1]
        if (
            'role' not in last_turn
            or 'content' not in last_turn
            or last_turn['role'] == 'assistant'
        ):
            return None
        else:
...
def convert_dataset_to_positive_samples(
    dataset: List[Dict], tokenizer: PreTrainedTokenizer, max_tokens: int
) -> List[Dict]:
    """
    ...
    for item in dataset:
        ...
        completion_tokens = tokenizer.encode(
            completion, max_length=max_tokens, add_special_tokens=False
        )[:max_tokens]
...
"""
    ...
    full_df.rename(
        columns={'question': 'prompt', 'task_type': 'source'}, inplace=True
    )
    ...
    for model_name in args.model_names:
        ...
        selected_df = filter_ds_by_group_size(
            full_df,
            group_by='source',
            group_size=int(args.task_size),
            seed=random.choice(range(1, 9999)),
        )
        ...
        sub_df = filter_ds_by_group_size(
            full_df,
            group_by='source',
            group_size=int(args.task_size * 0.2),
            seed=random.choice(range(1, 9999)),
        )
        ...
        logger.info(
            f'Start to generate {len(sub_ds)} positive samples with dual language...'
        )
        llm_positive_samples_dual_lang = generate_positive_samples(
            model,
            local_tokenizer,
            sub_ds,
            dual_lang_system_prompt,
            args.batch_size,
            max_tokens=args.max_tokens,
        )

        llm_positive_samples = (
            llm_positive_samples + llm_positive_samples_dual_lang
        )

        # always use the same tokenizer for negative samples
        llm_negative_samples = generate_negative_samples(
            llm_positive_samples, tokenizer, max_tokens=args.max_tokens
        )
...
def generate_strong_cot_samples(
    tokenizer: PreTrainedTokenizer, args: argparse.Namespace
) -> Dict[str, List[Dict]]:
    ...
    cot_ds_df = filter_ds_by_group_size(
        cot_ds_df,
        group_by='source',
        group_size=args.task_size,
        seed=random.choice(range(1, 9999)),
    )
    cot_dataset = list(cot_ds_df.to_dict(orient='records'))
    cot_positive_samples = convert_dataset_to_positive_samples(
        cot_dataset, tokenizer, max_tokens=args.max_tokens
    )
    cot_negative_samples = generate_negative_samples(
        cot_positive_samples, tokenizer, max_tokens=args.max_tokens
    )

    return {'positive': cot_positive_samples, 'negative': cot_negative_samples}


def generate_mixed_sft_samples(
    tokenizer: PreTrainedTokenizer, args: argparse.Namespace
) -> Dict[str, List[Dict]]:
    ...
    def last_message_valid(row):
        ...
        if isinstance(messages, list) and messages:
            ...
            return (
                ...
                and len(last_message.get('content', '')) > 200
            )
        return False
    ...
    mixed_positive_samples = convert_dataset_to_positive_samples(
        mixed_dataset, tokenizer, max_tokens=args.max_tokens
    )
    mixed_negative_samples = generate_negative_samples(
        mixed_positive_samples, tokenizer, max_tokens=args.max_tokens
    )

    return {
        'positive': mixed_positive_samples,
        'negative': mixed_negative_samples,
    }


def filter_ds_by_group_size(
    ds: pd.DataFrame, group_by: str, group_size: int, seed: int = 42
) -> pd.DataFrame:
    """Filter a dataset by group size."""
    return ds.groupby(group_by, group_keys=False).apply(
        lambda x: x.sample(n=min(len(x), group_size), random_state=seed)
    )
...
def main():
    ...
    mixed_sft_results = generate_mixed_sft_samples(
    ...
    for sample_dict in sample_list:
        ...
        train_samples.extend(
            [
                ...
                for item in neg_samples[:neg_split_index]
            ]
        )
        test_samples.extend(
            [
    ...
    stats = {
        ...
        'positive_train_size': len(
            [item for item in train_samples if item['label'] == 1]
        ),
        'positive_test_size': len(
            [item for item in test_samples if item['label'] == 1]
        ),
        'negative_train_size': len(
            [item for item in train_samples if item['label'] == 0]
        ),
        'negative_test_size': len(
            [item for item in test_samples if item['label'] == 0]
        ),
    }
    ...
    save_to_parquet_file(
        train_samples, f"{args.save_dir}/train.parquet", compression='snappy'
    )
    save_to_parquet_file(
        test_samples, f"{args.save_dir}/test.parquet", compression='snappy'
    )
